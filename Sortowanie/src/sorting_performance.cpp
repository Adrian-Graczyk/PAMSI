...
int main(int argc, char* argv[])
{
    ...
    /*std::vector<char const*> files = {"sort_data_10.bin","sort_data_100.bin","sort_data_1000.bin","sort_data_10000.bin",
    "sort_data_100000.bin","sort_data_1000000.bin","sort_data_10000000.bin","sort_data_100000000.bin","sort_data_1000000000.bin"};
    std::vector<char const*> data_size = {"10","100","1000","10000","100000","1000000","10000000","100000000","1000000000"};*/
  ...
  /* for(int i=0; i<files.size(); i++)
    {
        std::vector<int> tab;
        std::cout << "Quick sort for data size: "<<data_size[i]<<" WORST"<<std::endl;
        readData(files[i], tab);
        sort(tab.begin(), tab.end(), std::greater <>());
        timer.start();
        quickSort_worst(tab, 0, tab.size()-1);
        timer.stop();
        std::cout << "Sorting time: " << timer.sInterval() << std::endl<<std::endl;  
    }   
    ...
    {
        std::vector<int> tab;
        std::cout << "Quick sort for data size: "<<data_size[i]<<" Already Sorted"<<std::endl;
        readData(files[i], tab);
